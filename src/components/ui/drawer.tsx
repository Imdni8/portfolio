// Vendored from shadcn's base-nova registry (`npx shadcn add drawer`), with
// two project edits. Re-check both against upstream on `shadcn diff drawer`:
//   - `cn` comes from @/lib/utils, as in every other vendored primitive here.
//     The registry imports it from a standalone `cn` package, which the CLI
//     installed; it was removed again, since this project already has the
//     helper and one class merger is enough.
//   - The backdrop is the --overlay token (tokens.css), not Tailwind's stock
//     `bg-black/10` — stock palette steps are dark-only literals outside the
//     token system (see "Case studies" in CLAUDE.md).
